...
**Visual polish:** the default jet brightness/ceiling were retuned so the relativistically beamed jet reads with internal structure at low (pole-on) inclination instead of clipping to a solid white funnel. The Boyer–Lindquist polar axis (where g^φφ ∝ 1/sin²θ diverges) is now regularized with a small sin²θ floor, which bounds the divergence and removes the NaN-driven meridian seam present since Tier 1. **It does not make the axis correct**: rays entering the cone of half-angle √POLE_S2 ≈ 1.8° integrate on a deliberately wrong metric and terminate as captured, so a narrow black cone still runs out of the poles across regions that should show lensed disk. The floor is inert away from the axis, so `?parity` and `?shadow` are unchanged. A real fix (Kerr–Schild or Cartesian-momentum handoff near the axis) is tracked as a follow-up.
